...
class _TileCache:
    ...
    def id_from_name(self, name: str) -> int:
        ...
        if id_ is None:
            for key, data in game.const.tile_ids.DATA.items():
                if data["name"] == name:
                    id_ = key
    ...
    def data_from_id(id_: int) -> dict:
        ...
        return game.const.tile_ids.DATA[str(id_)]

    def data_from_name(self, name: str) -> dict:
        """Get tile data from its name."""
        return game.const.tile_ids.DATA[str(self.id_from_name(name))]

    @staticmethod
    def iter_names() -> typing.Generator[str, None, None]:
        """Iterate over tile names."""
        for data in game.const.tile_ids.DATA.values():
            yield data["name"]
