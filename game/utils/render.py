...
class _TileCache:
    ...
    def id_from_name(self, name: str) -> int:
        ...
        if id_ is None:
            for key, data in gamedata.tile_ids.TILE_IDS.items():
                if data["name"] == name:
                    id_ = key
    ...
    def data_from_id(id_: int) -> dict:
        ...
        return gamedata.tile_ids.TILE_IDS[str(id_)]

    def data_from_name(self, name: str) -> dict:
        """Get tile data from its name."""
        return gamedata.tile_ids.TILE_IDS[str(self.id_from_name(name))]

    @staticmethod
    def iter_names() -> typing.Generator[str, None, None]:
        """Iterate over tile names."""
        for data in gamedata.tile_ids.TILE_IDS.values():
            yield data["name"]
